void dibujarGrilla() {
  ...
  for (int i = 0; i < cant; i++) {
    for (int j = 0; j < cant; j++) {
      ...
      float mod = 438 / float(cant);
      float x = i * mod + mod / 2;
      float y = j * mod + mod / 2;

      if (cambioTam) { //hacer zoom en el tablero ;D
        float rect = map(mouseX, 0, width, tam * 1, tam * 2);
      }
      ...
      int tipo = (i + j) % 3; // 0, 1 o 2 para 3 tipos
...
void keyPressed() {
  if (key == '-') { //me alejo de la grilla
    ...
    cambiarColor = true;
    negro = color(random(255), random(255), random(255)); // tono fuerte aleatorio
    blanco = color(random(255), random(255), random(255)); // tono suave aleatorio!
  }
}
